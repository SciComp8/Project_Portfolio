...
##------Among cDEGs detected within the other 8 approaches, estimate the percentage of these cDEGs identified by one method across all random seed trials------
...
g <- ggplot(data = p.final, 
  ...
  labs(x = "Method", y = "Method", color = "Percentage \nof cDEGs") + 
  guides(size = "none") + 
  theme(legend.position = "right", 
        legend.key.height = unit(5, "line"),
        axis.text.y = element_text(color = "black"))

g.update <- plot_grid(g, ncol = 1, labels = LETTERS[3])
ggsave(filename = sprintf("../ApplicationResult/AddViz/shared_discovery/%s_%s_%s_dotplot.eps", date.analysis, var.name, "all_seed"),
       plot = g.update, device = cairo_ps, dpi = 600, width = 6, height = 6, units = "in")
