...
cnetplot.enrichResult <- function(x,
  ...
  if (!is.null(node_label_size))
    message(
      "node_label_size parameter has been changed to 'cex_label_category' and 'cex_label_gene'"
    )
  ...
  V(g)$size[1:n] <- size
  node_scales <-
    c(rep(cex_category, n), rep(cex_gene, (length(V(
      g
    )) - n)))
  if (colorEdge) {
    E(g)$category <- rep(names(geneSets), sapply(geneSets, length))
    edge_layer <- geom_edge(aes_(color =  ~ category), alpha = .8) # ALiu
  } else {
  ...
  if (!is.null(foldChange)) {
    ...
    V(g)$color <- "#B3B3B3"
    # sorted_indices <- order(substr(showCategory, 1, 1)) # ALiu
    V(g)$color[1:n] <- color_category # ALiu
    ...
    p <- p + edge_layer +
      ...
      scale_size(
        ...
        name = "Count",
        breaks = seq(min(x$Count[1:6]), max(x$Count[1:6]), 1)
      ) + # ALiu; range: a numeric vector of length 2 that specifies the minimum and maximum size of the plotting symbol after transformation
      ggnewscale::new_scale("size") +
  ...
  p <- p + theme_void() +
    ...
    guides(edge_color = guide_legend(order = 1)) # ALiu: put the legend of pathway category first
  ...
  if (node_label == "category") {
    ...
    if (utils::packageVersion("ggrepel") >= "0.9.0") {
      p <- p + geom_node_text(
        ...
        data = p$data[-c(1:n), ],
        repel = TRUE,
        size = label_gene * cex_label_gene,
        bg.color = "white"
      )
    } else {
      p <- p + geom_node_text(
...
cnetplot.compareClusterResult <- function(x,
  ...
  if (ncol(ID_Cluster_mat) > 1) {
    clusters <-
      match(colnames(ID_Cluster_mat), colnames(gene_Cluster_mat))
  ...
  if (is.null(x_loc))
    x_loc <- min(ID_Cluster_mat2$x)
  if (is.null(y_loc))
    y_loc <- min(ID_Cluster_mat2$y)
  #node_label
  if (node_label == "category") {
  ...
  if (ncol(ID_Cluster_mat2) > 4) {
    ...
    if (!is.null(foldChange)) {
      ...
      genes <-
        rownames(ID_Cluster_mat2)[(n + 1):nrow(ID_Cluster_mat2)]
      ...
      ID_Cluster_mat2$radius <-
        min(sizee) / 2 * gene_fc2 * sqrt(cex_gene)
      ID_Cluster_mat2$radius[1:n] <- sizee * sqrt(cex_category)
      ...
      p <- p + geom_scatterpie(
        ...
        cols = colnames(ID_Cluster_mat2)[1:(ncol(ID_Cluster_mat2) -
                                              3)],
        color = NA
      ) +
        geom_scatterpie_legend(
          ID_Cluster_mat2$radius[1:n],
          x = x_loc,
          y = y_loc + 3,
          n = legend_n,
          labeller = function(x)
            round(x ^ 2 * sum_yunion / cex_category)
        )  +
        geom_scatterpie(
          aes_(
            x =  ~ x,
        ...
        coord_equal() +
        geom_scatterpie_legend(
          ID_Cluster_mat2$radius[(n + 1):nrow(ID_Cluster_mat2)],
          x = x_loc,
          y = y_loc,
          n = legend_n,
          labeller = function(x)
            round(x * 2 / (min(sizee)) / sqrt(cex_gene), 3)
        ) +
