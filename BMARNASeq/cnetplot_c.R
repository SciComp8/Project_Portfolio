...
cnetplot.enrichResult <- function(x,
  ...
  if (!is.null(foldChange)) {
    ...
    p <- p + edge_layer +
      ...
      scale_size(range=c(3, 8) * cex_category, name = "Count", 
                 breaks = seq(min(x$Count[1:6]), max(x$Count[1:6]), 1)) + # ALiu; range: a numeric vector of length 2 that specifies the minimum and maximum size of the plotting symbol after transformation
      ggnewscale::new_scale("size") +
  ...
  p <- p + theme_void() + 
    scale_edge_color_manual(values = color_category, limits = showCategory, name = "Category", guide = guide_legend()) +  # ALiu
