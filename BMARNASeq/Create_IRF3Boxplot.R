...
# Not recommend a list of hand-written comparisons
# list(c("WBCno + BMIlow", "WBCyes + BMIlow"),
#      c("WBCno + BMIhigh", "WBCyes + BMIhigh"),
#      c("WBCno + BMIlow", "WBCno + BMIhigh"),
#      c("WBCyes + BMIlow", "WBCyes + BMIhigh"),
#      c("WBCyes + BMIlow", "WBCno + BMIhigh"),
#      c("WBClow + BMIlow", "WBCyes + BMIhigh"))


##------Make boxplots------
p1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI", test.type = "wilcox.test", y_transform = "log2")
p1.1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI", test.type = "wilcox.test", y_transform = "vst")

p2 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "MHABNWBC", test.type = "wilcox.test", y_transform = "log2")
p2.1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "MHABNWBC", test.type = "wilcox.test", y_transform = "vst")

p3 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI_MHABNWBC", 
                   test.type = "kruskal.test", y_transform = "log2")
p3.1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI_MHABNWBC", 
                   test.type = "kruskal.test", y_transform = "vst")

# pair_compare_obj <- compare_means(expr ~ BMI_MHABNWBC, data = IRF3.all, method = "wilcox.test")
# pair_compare_list <- extract_all_comparisons(pair_compare_obj)
# p3.1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI_MHABNWBC", 
#                      paired_comparisons_list = pair_compare_list,
#                      test.type = "kruskal.test")

p4 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "AGE_MHABNWBC", test.type = "kruskal.test")
p4.1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "AGE_MHABNWBC", 
                     paired_comparisons_list = pair_compare_list,
                     test.type = "kruskal.test")

p5 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI_AGE_SEX_MHABNWBC", test.type = "kruskal.test", y_transform = "log2")
p5.1 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "BMI_AGE_SEX_MHABNWBC", test.type = "kruskal.test", y_transform = "vst")

p6 <- make_boxplot(gene.symbol = "IRF3", data.type = "all", var.x = "AGE", test.type = "wilcox.test")


##------Put plots together in a panel------
g1 <- plot_grid(p1, p5, ncol = 2)
g1.1 <- plot_grid(p1.1, p5.1, ncol = 2)

g2 <- plot_grid(p2, p3, ncol = 2)
g2.1 <- plot_grid(p2.1, p3.1, ncol = 2)
...
ggsave(filename = sprintf("../ApplicationResult/AddViz/IRF3_BoxPlot/%s_%s_%s_%s.1.1.eps", date.analysis, "BMAseq", var.name, "all.seed"),
       plot = g1.1, device = cairo_ps, dpi = 600, width = 12, height = 7, units = "in")
...
ggsave(filename = sprintf("../ApplicationResult/AddViz/IRF3_BoxPlot/%s_%s_%s_%s.2.1.eps", date.analysis, "BMAseq", var.name, "all.seed"),
       plot = g2.1, device = cairo_ps, dpi = 600, width = 12, height = 6, units = "in")
