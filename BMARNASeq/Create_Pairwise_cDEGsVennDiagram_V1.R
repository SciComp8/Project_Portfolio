...
get_venndata <- function(seed.i = 8809678, 
                         ...
                         threshold = 5000,
                         ...){
  file.name.1 <- sprintf("../ApplicationData/derived/RandomSeed/Top5000/%sModel/%s%s%s.RData", 
  ...
  file.name.2 <- sprintf("../ApplicationData/derived/RandomSeed/Top5000/%sModel/%s%s%s.RData", 
...
plot_venn <- function(seed.i = 8809678, 
  ...
  usr.args <- match.call()[-1] # Extract argument values used in an R function call
  ...
  label_names <- c(paste0(ifelse(
    usr.args[["model.method.1"]] == "BMAseq",
    "BMAseq",
    ifelse(
      usr.args[["model.type.1"]] == "Multi",
      paste0(usr.args[["model.method.1"]], "_MVM"),
      paste0(usr.args[["model.method.1"]], "_UVM")
    )
  )),
  paste0(ifelse(
    usr.args[["model.type.2"]] == "Multi",
    paste0(usr.args[["model.method.2"]], "_MVM"),
    paste0(usr.args[["model.method.2"]], "_UVM")
  )))
...
file.name <- sprintf("../ApplicationResult/AddViz/VennDigram/Top5000/%s_%s.png", date.analysis, "BMI") # EPS does not support semi-transparency; tiff produces the MB-size file
