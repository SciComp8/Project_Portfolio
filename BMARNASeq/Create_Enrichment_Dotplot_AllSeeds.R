...
##------All approaches-----
# Show all cDEGs co-identified by all approaches that appear in at least 5 seeds
var.name <- "BMI"
threshold.i <- 5000
seed.vec <- c(8809678, 98907, 233, 556, 7890, 120, 2390, 778, 666, 99999)
class_freq_all_seed_list <- vector(mode = "list", length = 10L)
...
# Perform ORA analysis on cDEGs co-identified by all approaches that appear in at least 5 seeds
ora.obj <-
  enrichGO(
...
make_dotplot <- function(data.set = ora.obj.ordered, 
  ...
  p <- ggplot(data = data.set, 
    ...
    ggtitle(paste0("Shared cDEGs of ", var.name)) + 
    theme_bw(base_size = 14, base_family = "Arial") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          axis.text.y = element_text(size = 13, color = "black")) + 
    guides(size = guide_legend(order = 1)) # Order the discrete legend
