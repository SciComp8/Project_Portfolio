...
### All cases
# Loop every ranking threshold, every var.name, every seed, every model method
threshold.vec <- 5000
  ...
  for (var.i in var.vec) {
    for (seed.i in seed.vec) {
      for (model.method.i in model.method.vec) {
        ...
        ggsave(filename = file.name, plot = plot_panel, device = "png", dpi = 600, width = 12, height = 6, units = "in")        
      }
    }
  }
}
