...
easypackages::libraries("BMAseq", "limma", "qvalue", "parallel", "tidyverse",                         
                        ...
                        "org.Hs.eg.db", "latex2exp", "enrichplot", "patchwork",
                        "readxl", "pheatmap", "RColorBrewer", "gtsummary") |> suppressPackageStartupMessages()
...
loaded_data <- lapply(file_names, read_xlsx, na = c('NA'))
...
for (col in var_focus_2) {
  sum_dup <- sum(duplicated(pheno_data_filter[, col])) 
  if (sum_dup == row_count_minus_one) { 
    idx_identity <- c(idx_identity, which(names(pheno_data_filter) == col))
  }
}
idx_identity
...
1:nrow(pheno_data_final))
colnames(expr_data_filter)
