---
title: "Assessment of multi-model BMAseq and other single-model approaches on inferring robust differentially expressed genes"
author: "Anni Liu"
...
easypackages::libraries("BMAseq", "limma", "qvalue", "parallel", "tidyverse", 
                        ...
                        "org.Hs.eg.db", "latex2exp", "enrichplot", "patchwork") |> suppressPackageStartupMessages()
...
expr_data <- read.csv(paste0(data_dir, 'expr_data.csv')) # Load the RNA-seq data
...
# > TD: https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#principal-component-plot-of-the-samples
...
saveRDS(pheno_data_final, file=paste0(data_dir, 'pheno_data.RDS'))
saveRDS(expr_data_filter, file=paste0(data_dir, 'expr_data.RDS'))
...
## Rearrange cDEGs number boxplots
### All variables
```{r}
...
## Make cDEGs number box and line plots grouped by random seeds
### Each threshold in BMI
```{r}
##------Preprocess data------
cDEGs_num_uni <- readRDS("../ApplicationResult/Uni/RandomSeed/numcDEGs/Boxplot/boxplot.data.num.BMA.RDS")
...
for (threshold.i in threshold.vec) {
  ...
  g <- cDEGs_num_threshold |>
    ...
    if (threshold.i != 5000) {
      ggsave(
        filename = sprintf(
          "../ApplicationResult/AddViz/numcDEGs/%s_%s_boxline_%s.eps",
          date.analysis,
          var.i,
          threshold.i
        ),
  ...
  if (threshold.i != 5000) {
    ggsave(
      filename = sprintf(
        ...
        date.analysis, var.i, threshold.i
      ),
      plot = g,
      device = "eps",
      dpi = 600,
...
## Make cDEGs rate line and box plots grouped by random seeds
```{r}
##------Preprocess data------
cDEGs_rate_uni <- readRDS("../ApplicationResult/Uni/RandomSeed/ratecDEGs/Boxplot/boxplot.data.rate.BMA.RDS")
cDEGs_rate_uni <- cDEGs_rate_uni[cDEGs_rate_uni$method != "BMAseq", ]
...
for (threshold.i in threshold.vec) {
  ...
  if (threshold.i != 5000) {
    ggsave(
      filename = sprintf(
        "../ApplicationResult/AddViz/ratecDEGs/%s_%s_boxline_%s.eps",
        date.analysis, var.i, threshold.i
      ),
      plot = g,
      device = "eps",
...
## Make cDEGs mean rate line plots
```{r}
box::use(dplyr = dplyr)
##------Preprocess data------
cDEGs_rate_uni <- readRDS("../ApplicationResult/Uni/RandomSeed/ratecDEGs/Boxplot/boxplot.data.rate.BMA.RDS")
...
ggsave(
  filename = sprintf(
    "../ApplicationResult/AddViz/ratecDEGs/%s_%s_median_line_all_threshold.eps",
    date.analysis,
    var.i
  ),
  plot = g,
...
## Rearrange rediscovery rate plots
### All variables
```{r}
##------Load the rediscovery rate data modeled under the univariable/multivariable framework------
...
##------Refine the rediscovery rate data------
...
g2 <- duplicate_rate_all |>
  ...
  labs(y = "Mean duplicate rate of cDEGs", x = "Method") + 
  theme_BMA()
ggsave(filename = sprintf("../ApplicationResult/AddViz/RediscoveryRate/%s_mean.eps", date.analysis),
...
g2 <- duplicate_rate_bmi |>
  ...
  scale_y_continuous(labels = scales::label_number(accuracy = 0.01),
                     breaks = seq(0, 1, 0.01)) + 
  theme_BMA() + 
  scale_color_manual(values = c("#A73030FF", "#0073C2FF", "#003C67FF", "#8F7700FF", "#EFC000FF")) + # Avoid red-green combinations difficult for color-blind readers 
  labs(y = "Mean rediscovery rate of cDEGs", x = "Method", color = "Threshold") + 
  theme(legend.position = "right")

date.analysis <- format(Sys.Date(), "%Y%b%d")
ggsave(filename = sprintf("../ApplicationResult/AddViz/RediscoveryRate/%s_mean_each_threshold.eps", date.analysis),
       plot = g2, device = "eps", dpi = 600, width = 5, height = 5, units = "in")
```
...
## Make integrative heatmap and boxplot in unique and shared cDEGs between BMAseq and univariable or multivariable single model approaches
```{r}
...
## Make the consistent and unique of BMI-related cDEGs distribution boxplots between different approaches
```{r}
var.name <- c("BMI")
