...
ggsave(filename = sprintf("../ApplicationResult/AddViz/unique_discovery/%s_%s_%s.eps", date.analysis, var.name, "all_seed"),
       plot = p.u, device = cairo_ps, dpi = 600, width = 6, height = 7, units = "in")


# Combine the consistent and unique discovery
p.w <- p + p.u
ggsave(filename = sprintf("../ApplicationResult/AddViz/unique_discovery/%s_%s_%s.eps", date.analysis, var.name, "all_seed"),
       plot = p.w, device = cairo_ps, dpi = 600, width = 12, height = 7, units = "in")
