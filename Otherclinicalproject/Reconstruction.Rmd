...
```{r echo=FALSE, warning=FALSE}
covariates <- var_focus[-which(var_focus %in% c("Chemotherapy", "Endocrine.therapy", "Radiation", "Bilateral.cancer", "Bilateral.risk.reducing", "Plastics.referral", "Reconstruction.type", "Diagnosis", "Reconstruction"))]
...
t_merge <- tbl_merge(tbls = list(t_uv_2, t_mv),
...
# Trend of the postmastectomy reconstruction rate over time
```{r echo=FALSE, warning=FALSE}
...
Reconstruction_data <- data.frame(Year = line_plot_data$Procedure.year |> unique(), Reconstruction_rate = Reconstruction_rate)

ggplot(data = Reconstruction_data, 
       mapping = aes(x = Year, y = Reconstruction_rate)) +
  geom_line(aes(group = 1), color = "#0073C2FF") +
  theme_minimal() + 
