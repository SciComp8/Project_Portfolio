...
##------New update------
# Use the Axillary micromets v3.xlsx dataset, which excludes patients with bilateral cancer at the time of diagnosis and those who received neoadjuvant therapy. The sample size is 102.

##------Previous update------
# Use the univariate log-rank test and Cox-proportional hazards model to investigate the association between each potential predictor (i.e., age, gender, adjuvant radiation) on the composite outcomes of interest (i.e., axillary/local/distant recurrence and death)
## Review the distributions of recurrence and death
...
## ARDeath: 1 if Axillary_recurrence_2 == "Yes" or Death_2 == "Yes"
##          0 if Axillary_recurrence_2 == "No" and Death_2 == "No"
##          NA if Axillary_recurrence_2 == NA
## LRDeath: 1 if Local_recurrence_2 == "Yes" or Death_2 == "Yes"
##          0 if Local_recurrence_2 == "No" and Death_2 == "No"
##          NA if Local_recurrence_2 == NA
## DRDeath: 1 if Distant_recurrence_2 == "Yes" or Death_2 == "Yes"
##          0 if Distant_recurrence_2 == "No" and Death_2 == "No"
##          NA if Distant_recurrence_2 == NA
## T2ARDeath: `Time_to_AXR` if Axillary_recurrence_2 == "Yes" and Death_2 == "No"
##            `Time_to_AXR` if Axillary_recurrence_2 == "Yes" and Death_2 == "Yes"
##            `Time_to_death` if Death_2 == "Yes" and Axillary_recurrence_2 == "No"
##            `Follow_up_time..days.` if ARDeath == 0
##            NA if ARDeath == NA
## T2LRDeath: `Time_to_LR` if Local_recurrence_2 == "Yes" and Death_2 == "No"
##            `Time_to_LR` if Local_recurrence_2 == "Yes" and Death_2 == "Yes"
##            `Time_to_death` if Death_2 == "Yes" and Local_recurrence_2 == "No"
##            `Follow_up_time..days.` if LRDeath == 0
##            NA if LRDeath == NA
## T2DRDeath: `Time_to_DR` if Distant_recurrence_2 == "Yes" and Death_2 == "No"
##            `Time_to_DR` if Distant_recurrence_2 == "Yes" and Death_2 == "Yes"
##            `Time_to_death` if Death_2 == "Yes" and Distant_recurrence_2 == "No"
##            `Follow_up_time..days.` if DRDeath == 0
##            NA if DRDeath == NA
## Note: 1 means the composite endpoint occurs; 0 means the censoring occurs
```
...
##################################################################
dat0 = dat.work = dget("../data/derived/2022Dec24_dat_micromets.Rdata")
```
...
In our univariate survival analysis, the axillary/local/distant recurrence-free survival time is defined as a composite endpoint which combines the time to axillary/local/distant recurrence, time to death, and time to last follow-up into a univariate time to the first event or censoring. To illustrate how this combination of time works, we enumerate different scenarios for calculating the axillary recurrence-free survival time. The same logic applies to the other two types of recurrence (e.g., local recurrence and distant recurrence).
...
dat0 <- readxl::read_xlsx(
  "../data/raw/Axillary micromets\ v3.xlsx", # \ escape
  sheet = "Coded v3",
  range = c("A1:BA129"),
  col_names = T,
...
##################################################################
##                Recode ARDeath/LRDeath/DRDeath                ##
##################################################################
table(dat0$Axillary_recurrence_2 , dat0$Death_2, useNA = "ifany")
  #       No Yes
  # No   115   5
...
##################################################################
##             Recode T2ARDeath/T2LRDeath/T2DRDeath             ##
##################################################################
dat0 <- dat0 %>%
  mutate(
    T2ARDeath = case_when(
...
summary(dat0$T2ARDeath)
   ...
   #  122    1337    2706    2780    4017    6230       6 
...
summary(dat0$T2LRDeath)
   # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
   #  122    1199    2536    2696    3941    6230       6 
...
summary(dat0$T2DRDeath)
   # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
   #  122    1193    2573    2704    3904    6230       5 
...
##################################################################
dat.work <- dget("../data/derived/2022Dec24_dat_micromets.Rdata")
```
...
vars.all <- c(
  ...
  "Axillary_recurrence_2", "Local_recurrence_2", "Distant_recurrence_2",
  "New_breast_cancer_2"
)
...
In our univariate survival analysis, the axillary/local/distant recurrence-free survival time is defined as a composite endpoint which combines the time to axillary/local/distant recurrence, time to death, and time to last follow-up into a univariate time to the first event or censoring. To illustrate how this combination of time works, we enumerate different scenarios for calculating the axillary recurrence-free survival time. The same logic applies to the other two types of recurrence (i.e., local recurrence and distant recurrence).
...
# |**Age_dx_grp_1**            |            |        |        |        |P=0.394 |
# |40-49                       |reference   |        |        |        |        |
# |20-29                       |1.00e+00    |0.00    |Inf     |P=1.000 |        |
# |30-39                       |3.06e+08    |0.00    |Inf     |P=0.999 |        |
# |50-59                       |3.79e+08    |0.00    |Inf     |P=0.999 |        |
# |60-69                       |4.63e+08    |0.00    |Inf     |P=0.999 |        |
# |70-79                       |7.29e+08    |0.00    |Inf     |P=0.999 |        |
# |80-89                       |NA          |NA      |NA      |P=   NA |        |
# |90+                         |NA          |NA      |NA      |P=   NA |        |
...
# |**Age_dx_grp_1**            |            |        |        |        |P=0.394 |
# |50-59                       |reference   |        |        |        |        |
# |20-29                       |0.00        |0.00    |Inf     |P=0.999 |        |
# |30-39                       |0.81        |0.07    |8.91    |P=0.861 |        |
# |40-49                       |0.00        |0.00    |Inf     |P=0.999 |        |
# |60-69                       |1.22        |0.17    |8.69    |P=0.841 |        |
# |70-79                       |1.92        |0.27    |13.71   |P=0.514 |        |
# |80-89                       |NA          |NA      |NA      |P=   NA |        |
# |90+                         |NA          |NA      |NA      |P=   NA |        |
...
# |70-79                       |reference   |        |        |        |        |
# |20-29                       |0.00        |0.00    |Inf     |P=0.999 |        |
# |30-39                       |0.42        |0.04    |4.65    |P=0.479 |        |
# |40-49                       |0.00        |0.00    |Inf     |P=0.999 |        |
# |50-59                       |0.52        |0.07    |3.70    |P=0.514 |        |
# |60-69                       |0.64        |0.09    |4.53    |P=0.651 |        |
# |80-89                       |NA          |NA      |NA      |P=   NA |        |
# |90+                         |NA          |NA      |NA      |P=   NA |        |
...
# |Non-hispanic black          |5.32      |1.03    |27.52   |P=0.046* |        |
# |Asian                       |0.00      |0.00    |Inf     |P=0.999 |        |
# |Hispanic                    |0.00      |0.00    |Inf     |P=0.999 |        |
# |Other                       |NA        |NA      |NA      |P=   NA |        |
# |*var.missing*               |          |        |        |        |P=0.466 |
# |non.missing                 |reference |        |        |        |        |
# |missing                     |0.00      |0.00    |Inf     |P=0.999 |        |

# dat.work$Race_2 <- relevel(dat.work$Race_2, ref = "Non-hispanic black")
# |**Race_2**                  |          |        |        |        |P=0.105 |
# |Non-hispanic black          |reference |        |        |        |        |
# |Non-hispanic white          |0.19      |0.04    |0.97    |P=0.046 |        |
# |Asian                       |0.00      |0.00    |Inf     |P=0.999 |        |
# |Hispanic                    |0.00      |0.00    |Inf     |P=0.999 |        |
# |Other                       |NA        |NA      |NA      |P=   NA |        |
...
# |**Race_2**                  |          |        |        |        |P=0.105 |
# |Asian                       |reference |        |        |        |        |
# |Non-hispanic black          |2.20e+08  |0.00    |Inf     |P=0.999* |        |
# |Non-hispanic white          |4.14e+07  |0.00    |Inf     |P=0.999 |        |
# |Hispanic                    |1.00e+00  |0.00    |Inf     |P=1.000 |        |
# |Other                       |NA        |NA      |NA      |P=   NA |        |
# |*var.missing*               |          |        |        |        |P=0.466 |
# |non.missing                 |reference |        |        |        |        |
```
...
for(i in 1:length(vars.ana)) {
  var.name <- vars.ana[i]
  var.self <- dat.work[, var.name]
  var.missing <- ifelse(is.na(var.self), "missing", "non.missing") %>%
    factor(levels = c("non.missing", "missing"))
  model.exp <- paste0("Surv(T2ARDeath, ARDeath) ~ ", var.name)
  model.exp.miss <- "Surv(T2ARDeath, ARDeath) ~ var.missing"
  tmp <- coxph(formula(model.exp), data = dat.work)
  if(sum(var.missing == "missing") > 0) {
    tmp2 <- coxph(formula(model.exp.miss), data = dat.work)
    out[[i]] <- list(tmp, tmp2)
  } else {
    out[[i]] <- list(tmp)
  }
...
for(i in 1:length(vars.ana)) {
  ...
  var.missing <- ifelse(is.na(var.self), "missing", "non.missing") %>%
    factor(levels = c("non.missing", "missing"))
  model.exp <- paste0("Surv(T2LRDeath, LRDeath) ~ ", var.name)
  model.exp.miss <- "Surv(T2LRDeath, LRDeath) ~ var.missing"
  tmp <- coxph(formula(model.exp), data = dat.work)
  if(sum(var.missing == "missing") > 0) {
    tmp2 <- coxph(formula(model.exp.miss), data = dat.work)
    out[[i]] <- list(tmp, tmp2)
...
for(i in 1:length(vars.ana)) {
  ...
  var.missing <- ifelse(is.na(var.self), "missing", "non.missing") %>%
    factor(levels = c("non.missing", "missing"))
  model.exp <- paste0("Surv(T2DRDeath, DRDeath) ~ ", var.name)
  model.exp.miss <- "Surv(T2DRDeath, DRDeath) ~ var.missing"
  tmp <- coxph(formula(model.exp), data = dat.work)
  if(sum(var.missing == "missing") > 0) {
    tmp2 <- coxph(formula(model.exp.miss), data = dat.work)
    out[[i]] <- list(tmp, tmp2)
...
##################################################################
## Investigate if the proportional hazards assumption of univariate log-rank test and Cox-proportional hazards model is satisfied
## Think about the possibility to incorporate the proportional hazards assumption test into fplot.surv() in BTKR package
