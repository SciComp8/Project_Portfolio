...
dat.work <- read_fst(path = "../data/derived/2024Aug15_dat_TNBC.RData")

a <- subset(dat.work, SBE.event == 1)
table(a$Any.Attempt.at.Lumpectomy, useNA = "ifany")
table(Mastectomy=a$Mastectomy.Surgery, CPM=a$CPM, useNA = "ifany")

table(a$Mammo.Screen.Detected, useNA = "ifany")
table(a$MRI.Screen.Detected, useNA = "ifany")
table(a$Histology.Subsequent, useNA = "ifany")

b <- subset(dat.work, Genetic.Testing.Results %in% c("BRCA1", "BRCA2", "BRCA1/2", "BRCA"))
table(b$SBE.event, useNA = "ifany")
c <- subset(dat.work, Genetic.Testing.Results == "BRCA1/2 negative")
table(c$SBE.event, useNA = "ifany")

d <- subset(a, Mastectomy.Surgery %in% c("No", "Yes") & CPM == "No")
table(Ipsilateral=d$Past.Ipsilateral.Br.CA, Contralateral=d$Past.Contralateral.Br.CA, useNA = "ifany")
d[which(d$Past.Ipsilateral.Br.CA=="Yes" & d$Past.Contralateral.Br.CA=="Yes"), ]$ID

e <- subset(a, CPM == "Yes")
table(e$SBE.HR, useNA ="ifany")
table(e$SBE.ER, e$SBE.PR, e$SBE.HER2, useNA ="ifany")
subset(e, SBE.ER=="SBEYesERNeg" & SBE.PR=="SBEYesPRNeg" & SBE.HER2=="SBEYesHER2Neg") |> nrow()
subset(e, SBE.HR=="SBEYesHRPos") |> nrow()
```
...
# (sapply(c("T2SBE", "T2Death", "T2LFU", "T2ER", "T2PR", "T2HER2", "T2HR"), function(x) with(dat0, summary(get(x)))))
...
# Systematic thinking:
#1: Unsure in which year SBE occurred
#2: Unsure if SBE occurred in 3 years
#2: Unsure if SBE occurred in 3 years
#2: Unknown SBE status
...
# Systematic thinking:
#1: Unsure in which year SBE+HR+ occurred
#2: Unsure if SBE+HR+ occurred in 3 years
#2: Unsure if SBE+HR+ occurred in 3 years
#2: Unknown SBE+HR+ status
...
```{r}
dat.work <- read_fst(path = "../data/derived/2024Aug08_dat_TNBC.RData")
```
